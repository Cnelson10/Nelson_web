...
public class ValidationUtils {
    ...
    public static String decimalValidationMsg(String val, boolean required) {
        System.out.println(val + " if it has a $, lets remove it and print it out again.");
        val = val.replace("$", "");
    ...
    public static String stringValidationMsg(String val, int maxlen, boolean required) {
        ...
        if (val.length() > maxlen) {
            return "Please shorten to " + maxlen + " characters. e.g. [" + val.substring(0, maxlen) + "]";
        } else {
            return ""; // input is good
        }
    }
    
    public static String yearValidationMsg(String val, int maxlen, boolean required) {
        // System.out.println("*************trying to convert ["+val+"] to date");

        if (val == null) {
            return "ValidationUtils.yearValidationMsg(): Programmer error - should not be trying to validate null.";
        }
        if ((val.length() == 0) && !required) {
            return "";  // Since this field is not required, empty string is valid user entry.
        }
        if (val.length() > maxlen) {
            return "Please shorten to " + maxlen + " characters. e.g. [YYYY] or to 1 or 2 characters [0 - 99]";
        } else if (val.length() == 3){
            return "Please enter a year of " + maxlen + " characters. e.g. [YYYY] or of 1 or 2 characters [0 - 99]";
        } else if (val.length() == 4){
            if((Integer.parseInt(val) < 1901) || (Integer.parseInt(val) > 2019)){
                return "Please enter a year within the range of 1901 and 2019";
            } else {
                return "";
            }
        } else {
